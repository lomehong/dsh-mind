...
import { appendRollup, buildRollupPrompt, selectRollupSpan } from "./rollups.js";
import { readAllSteps } from "./recap.js";
/** 尝试做一轮摘要（异步尽力而为；调用方 void 掉，绝不 await 主流程）。 */
export async function tryRollup(opts) {
    try {
        ...
        const s2 = loadState();
        if ((s2.lastRolledUpSeq ?? 0) >= selection.upToSeq)
...
/** 步骤数守卫（测试/外部调用用）。 */
export function countUnrolled(steps, lastRolledUpSeq) {
