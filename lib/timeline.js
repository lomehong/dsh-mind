...
export function readTail(maxSteps) {
    const file = timelinePath();
    if (!existsSync(file))
        return { steps: [], skippedBadTail: 0 };
    ...
    for (let i = lines.length - 1; i >= 0 && steps.length < maxSteps; i--) {
        try {
            ...
            if (i === lines.length - 1)
                skippedBadTail += 1; // 只容忍坏尾行（崩溃残留）
            else
                break; // 中段坏行视为文件损坏，停止回读
        }
    }
...
export function paginateSteps(steps, limit, beforeSeq) {
 ...
 */
export function archiveOldSteps(cutoffDays, now = new Date()) {
    const file = timelinePath();
    if (!existsSync(file))
        return 0;
