...
import { isTransientServiceError, WakeRunner } from "./runner.js";
...
import { dueEscalations, openAsk, parseAskPayload, parseAskSettleIds, settleAsk, settleAsksByGoal } from "./asks.js";
    ...
    async function runWake(trigger) {
        ...
        if (settleIds.length > 0) {
            ...
            for (const id of settleIds) {
                if (before.some(a => a.id === id))
                    logger.info?.(`[dsh-mind] 请求单已结清（${id}）：TA 显式销账（前提已消失/另行办结）`);
            }
        }
