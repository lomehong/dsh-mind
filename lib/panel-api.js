...
import { readTail, paginateSteps } from "./timeline.js";
...
import { loadMissions, saveMissions } from "./missions.js";
    ...
    web.register({
        ...
        handler: (req, res) => {
            ...
            json(res, 200, {
                ...
                openAsks: Array.isArray(st.openAsks) ? st.openAsks.filter(a => a?.state === 'open').length : 0, // P5 请求账（等待主人的解锁请求，§6.5）
                openAskPreview: (Array.isArray(st.openAsks) ? st.openAsks.filter(a => a?.state === 'open') : [])
                    .slice().sort((x, y) => Date.parse(x.ts) - Date.parse(y.ts))[0]?.what.slice(0, 24),
                quiet: {
        ...
        handler: (req, res) => {
            ...
            void (async () => {
                try {
                    ...
                    if (id === '')
                        return json(res, 400, { ok: false, error: '缺少请求单 id' });
                    if (answer.length === 0)
                        return json(res, 400, { ok: false, error: '答复内容不能为空' });
                    if (answer.length > 2000)
                        return json(res, 400, { ok: false, error: '一次最多答复 2000 字' });
                    const settled = deps.answerAsk(id, answer);
                    if (!settled)
                        return json(res, 404, { ok: false, error: '请求单不存在或已结清' });
                    json(res, 200, { ok: true });
                }
                catch (e) {
                    json(res, 400, { ok: false, error: e instanceof Error ? e.message : String(e) });
    ...
    web.register({
        ...
        handler: (req, res) => {
            if (!sameOrigin(req))
                return json(res, 403, { ok: false, error: 'cross-origin denied' });
            if (req.method === 'GET')
                return json(res, 200, { ok: true, missions: loadMissions() });
