...
export function ticksForRange(level, startMs, endMs) {
    ...
    if (level === 'year') {
        ...
        while (cur.getTime() <= endMs) {
            ...
            push(t, firstOfDay ? `${prefix} ${cur.getHours()}时` : `${cur.getHours()}时`, cur.getHours() === 0);
            lastDay = day;
