...
import { RECAP_FANOUT } from './recap.ts'
...
export interface RollupRecord {
  ...
  ts: string
  /** 覆盖的原始步骤 seq 区间（闭区间） */
  refs: { steps: [number, number] }
  span: number
  /** LLM 摘要正文（≤200 字） */
  text: string
}

export function rollupsPath(): string {
  return join(mindHome(), 'rollups.jsonl')
}

export function appendRollup(rec: RollupRecord): void {
...
/** 读最近 n 条（新→旧；坏行跳过）。 */
export function readRollups(n = 12): RollupRecord[] {
  const file = rollupsPath()
  if (!existsSync(file)) return []
  const out: RollupRecord[] = []
  for (const line of readFileSync(file, 'utf8').split('\n')) {
    if (line.trim() === '') continue
    try {
      const rec = JSON.parse(line) as RollupRecord
      if (rec?.v === 1 && typeof rec.text === 'string' && rec.refs?.steps?.length === 2) out.push(rec)
    } catch { /* 坏行跳过 */ }
  }
...
export function selectRollupSpan(
  steps: ReadonlyArray<TimelineStep>,
  lastRolledUpSeq: number,
  fanout = RECAP_FANOUT,
): { span: TimelineStep[]; upToSeq: number } | null {
  const pending = steps.filter(s => typeof s.seq === 'number' && s.seq > lastRolledUpSeq)
  if (pending.length < fanout) return null
  const span = pending.slice(0, fanout)
  return { span, upToSeq: span.at(-1)!.seq! }
}

/** 摘要提示词（纯函数；低配会话用——只许输出摘要本身）。 */
export function buildRollupPrompt(span: ReadonlyArray<TimelineStep>): string {
  const lines = span.map(s =>
    `[#${s.seq} ${s.ts.slice(5, 16).replace('T', ' ')} ${s.type}${s.fn !== undefined ? `/${s.fn}` : ''}] ${s.content.slice(0, 200)}`)
  return [
