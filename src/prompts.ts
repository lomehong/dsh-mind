...
import { DEFAULT_PROMPT_BLOCKS, type PromptBlocks } from './wake-prompt.ts'

export type PromptBlockKey = 'menu' | 'rules' | 'outputFormat' | 'style'

export type PromptOverrides = Partial<Record<PromptBlockKey, string>>

export const PROMPT_BLOCK_KEYS: ReadonlyArray<PromptBlockKey> = ['menu', 'rules', 'outputFormat', 'style']
...
export function loadPromptOverrides(): PromptOverrides {
  ...
  try {
    ...
    for (const key of PROMPT_BLOCK_KEYS) {
      const v = raw[key]
      if (typeof v === 'string' && v.trim() !== '') out[key] = v.slice(0, MAX_BLOCK_CHARS)
    }
    return out
  } catch {
    return {}
...
export function resolveWakePromptBlocks(overrides: PromptOverrides): PromptBlocks {
  return {
...
/** 当前生效四块（便捷封装：读覆盖层并解析）。 */
export function resolveCurrentBlocks(): PromptBlocks {
