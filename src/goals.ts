...
/** 判断内容形态（纯函数）。 */
export function goalMarkerOf(content: string): 'active' | 'done' | 'dropped' | null {
  const t = content.trimStart()
  if (t.startsWith('[目标·完成]')) return 'done'
  if (t.startsWith('[目标·放弃]')) return 'dropped'
  if (t.startsWith('[目标]')) return 'active'
  return null
}

/** 从记忆条目里提取目标（纯函数）。entries: {content, ts?, seq?}[]。 */
export function extractGoalEntries(
  entries: ReadonlyArray<{ content: string; ts?: string; seq?: number }>,
): GoalEntry[] {
  const out: GoalEntry[] = []
  for (const e of entries) {
    if (goalMarkerOf(e.content) !== 'active') continue
...
export function settleGoals(
  active: ReadonlyArray<GoalEntry>,
  entries: ReadonlyArray<{ content: string; ts?: string }>,
): GoalEntry[] {
