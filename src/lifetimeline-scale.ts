...
/** 视口跨度 → 语义粒度。 */
export function levelForSpan(spanMs: number): ScaleLevel {
...
export function bucketStart(level: ScaleLevel, t: number): number {
  const d = new Date(t)
  switch (level) {
...
/** 桶的下一边界（用于点击下钻的视口范围）。 */
export function nextBucketStart(level: ScaleLevel, bucketT: number): number {
...
export interface Tick { t: number; label: string; major?: boolean }

/** 可视范围内的刻度线（含标签；本地时区；粒度语义化）。
 *  包含覆盖可视起点的那个标签（左端第一段也要有名字——友好性）；
 *  跨天的小时粒度：首刻度与零点刻度带日期前缀，零点为主刻度。 */
export function ticksForRange(level: ScaleLevel, startMs: number, endMs: number): Tick[] {
  const out: Tick[] = []
  const from = bucketStart(level, startMs)
  const push = (t: number, label: string, major = false): void => { if (t >= from && t <= endMs) out.push({ t, label, major }) }
  if (level === 'year') {
