 ...
 * 生活流（「TA 的一天」）与照护折叠为陪衬；右上角可切「工程视图」（EngView）。
 */
...
import { groupByDay, greeting, type NarratedStep } from '../narrate.ts'
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <div style={{
          ...
          <div style={{ fontSize: 11, color: 'var(--dsw-alias-label-secondary, #888)', marginBottom: 2 }}>你 · {fmtClock(step.ts)}</div>
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
        <div style={{
          ...
          <div style={{ fontSize: 11, color: 'var(--dsw-alias-label-secondary, #888)', marginBottom: 2 }}>TA · {fmtClock(step.ts)}</div>
          <div style={{ fontSize: 13, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{step.body}</div>
        </div>
      </div>
    )
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      {groups.map(day => (
        <div key={day.label} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--dsw-alias-label-secondary, #888)', fontSize: 12, padding: '6px 0' }}>
    ...
    <details>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 12.5, padding: '10px 0', color: 'var(--dsw-alias-label-secondary, #bbb)' }}>
        ...
        <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
          ...
          {!stopped && !status.quiet?.active && status.wakeAt > Date.now() && (
            <span>下次自己醒：约 {Math.max(1, Math.round((status.wakeAt - Date.now()) / 60000))} 分钟后</span>
          )}
        </div>
...
export function MindPage(): JSX.Element {
  ...
  const refreshAll = (): void => { status.refresh(); feed.refresh() }
  if (view === 'eng') {
    return (
      <div style={{ padding: 16 }}>
        ...
        <EngView status={status.data} refresh={refreshAll} />
      </div>
    )
  }
    ...
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px 40px', display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, padding: '18px 0 6px' }}>
        ...
        <div style={{ fontSize: 12.5, color: 'var(--dsw-alias-label-secondary, #888)' }}>
          {greeting(new Date())}。TA 就住在这台 dsh 里——右下角也一直有 TA。
        </div>
      </div>
      <Composer status={st} refresh={refreshAll} />
      <details>
        <summary style={{ cursor: 'pointer', fontSize: 13, color: 'var(--dsw-alias-label-secondary, #aaa)', userSelect: 'none' }}>TA 的一天</summary>
        <div style={{ paddingTop: 10 }}>
          <LifeStream feed={feed.data} />
        </div>
      </details>
      <CareDrawer status={st} refresh={refreshAll} />
    </div>
  )
}
