...
export function settleAsk(list: ReadonlyArray<AskEntry>, id: string): AskEntry[] {
  return list.filter(a => a.id !== id)
}

/** 关联目标销账联动：goalTitle ∈ settledTitles 的 open 请求自动结清（§6.5）。 */
export function settleAsksByGoal(list: ReadonlyArray<AskEntry>, settledTitles: ReadonlyArray<string>): AskEntry[] {
  const titles = new Set(settledTitles.map(t => t.trim()).filter(t => t !== ''))
  if (titles.size === 0) return [...list]
  return list.filter(a =>
    !(a.state === 'open' && a.goalTitle !== undefined && titles.has(a.goalTitle.trim())))
}

/** open 请求视图（运行时状态各处只消费 open；坏条目守卫在 state.ts 装载层）。 */
export function openAsksOf(list: ReadonlyArray<AskEntry>): AskEntry[] {
  return list.filter(a => a !== null && typeof a === 'object' && a.state === 'open' && typeof a.what === 'string')
}

/** 久悬请求（age > thresholdHours；纯函数）。 */
export function staleAsks(list: ReadonlyArray<AskEntry>, nowMs: number, thresholdHours = 24): AskEntry[] {
...
export function dueEscalations(
  ...
  return staleAsks(list, nowMs, thresholdHours).filter(a => {
    if (a.lastEscalatedAt === undefined) return true
...
export function parseAskPayload(text: string): { what: string; why?: string; howto?: string; goalTitle?: string } {
  ...
  for (const p of parts.slice(1)) {
    if (why === undefined && /^为了[:：]/.test(p)) why = p.slice(3).trim() || undefined
    else if (howto === undefined && /^给了之后[:：]/.test(p)) howto = p.slice(5).trim() || undefined
    else if (goalTitle === undefined && /^目标[:：]/.test(p)) goalTitle = p.slice(3).trim().slice(0, 200) || undefined
  }
  return {
    what,
...
export function parseAskSettleIds(text: string): string[] {
  const ids: string[] = []
