 ...
 * turn/end）。
 */
import { GatewayClient, sessionAddress, type SessionPage, type SessionSummary } from './gateway.ts'
...
export class WakeRunner {
  ...
  async ensureSession(currentId: string | undefined, lastTokensIn: number): Promise<{ sessionId: string; reset: boolean }> {
    if (currentId !== undefined && lastTokensIn < this.opts.resetThresholdTokens) {
      try {
        const list = (await this.gw.invoke('session', 'list')) as { items?: ReadonlyArray<SessionSummary> }
  ...
  async runWake(sessionId: string, prompt: string): Promise<WakeRunResult> {
    ...
    for (;;) {
      if (Date.now() > deadline) throw new Error(`mind wake run timed out after ${this.opts.timeoutMs}ms`)
  ...
  private async inspectOnce(sessionId: string, startedAt: number): Promise<SessionPage | undefined> {
    ...
    try {
      ...
      if (follow === undefined || follow.type !== 'snapshot' || typeof follow.cursor !== 'number') return undefined
