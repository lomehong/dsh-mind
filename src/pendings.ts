...
export function pushPending(
  list: ReadonlyArray<PendingEntry>,
  entry: PendingEntry,
  cap = 20,
): PendingEntry[] {
...
export function resolvePendingsBefore(
  list: ReadonlyArray<PendingEntry>,
  wakeStartTs: string,
): PendingEntry[] {
...
export function stalePendings(
  list: ReadonlyArray<PendingEntry>,
  nowMs: number,
  thresholdHours = 24,
  limit = 3,
): Array<{ ageHours: number; text: string }> {
  const out: Array<{ ageHours: number; text: string }> = []
  for (const p of list) {
