...
import { mindHome, type MindConfig } from './config.ts'
...
export function topKeywords(texts: ReadonlyArray<string>, limit = 5): string[] {
  const freq = new Map<string, number>()
  const push = (kw: string): void => {
    if (kw.length < 2 || kw.length > 24 || STOPWORDS.has(kw)) return
...
function mixOf(steps: ReadonlyArray<TimelineStep>): string {
...
/** 对一步序列做一层聚合（纯函数）。 */
export function rollupSpan(steps: ReadonlyArray<TimelineStep>): RollupEntry {
  ...
  const contents = steps.map(s => {
    const anyS = s as unknown as { content?: unknown; mix?: string; keywords?: string[] }
    if (typeof anyS.content === 'string') return anyS.content
    return [anyS.mix, ...(anyS.keywords ?? [])].filter(Boolean).join(' ')
  }).filter(c => c.trim() !== '')
  const lastWake = [...steps].reverse().find(s => (s as unknown as { final?: unknown }).final !== undefined || s.type === 'wake')
  const lastFinal = lastWake !== undefined
    ? ((lastWake as unknown as { final?: string; lastFinal?: string }).final ??
       (lastWake as unknown as { lastFinal?: string }).lastFinal)
    : undefined
  return {
    span: steps.length,
    from: steps[0]?.ts ?? '',
    to: steps.at(-1)?.ts ?? '',
    mix: mixOf(steps.map(s => ({ ...s, type: (s as unknown as { type?: string }).type ?? (s as unknown as { span?: number }).span?.toString() ?? 'rollup' } as TimelineStep))),
...
export function recapTiers(steps: ReadonlyArray<TimelineStep>, fanout = RECAP_FANOUT): RollupEntry[][] {
  const tiers: RollupEntry[][] = []
  let current = [...steps]
  while (current.length >= fanout) {
    const next: RollupEntry[] = []
    for (let i = 0; i + fanout <= current.length; i += fanout) {
      next.push(rollupSpan(current.slice(i, i + fanout)))
    }
    tiers.push(next)
    current = next as unknown as TimelineStep[]
  }
  return tiers
}

function formatTierLine(tierIndex: number, entry: RollupEntry): string {
...
export function readAllSteps(): TimelineStep[] {
  const file = join(mindHome(), 'timeline.jsonl')
  if (!existsSync(file)) return []
  const steps: TimelineStep[] = []
  for (const line of readFileSync(file, 'utf8').split('\n')) {
    if (line.trim() === '') continue
    try { steps.push(JSON.parse(line) as TimelineStep) } catch { /* 坏行跳过 */ }
  }
  return steps
}
