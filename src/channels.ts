...
const channels = new Map<string, MindChannel>()

/** 注册投递通路（幂等：同 id 覆盖）。返回注销函数。 */
export function registerMindChannel(channel: MindChannel): () => void {
  channels.set(channel.id, channel)
  return () => { channels.delete(channel.id) }
}

export function registeredChannels(): MindChannel[] {
...
export async function deliverToChannels(payload: MindChannelDeliverPayload): Promise<DeliverReport> {
  const results: DeliverReport['results'] = []
  let delivered = 0
  for (const ch of registeredChannels()) {
    try {
      const ok = await ch.deliver(payload)
      if (ok === true || ok === undefined) delivered += 1
